...
(function($)
{
	...
	{
		$("#main-header nav a[href='"+ location.pathname +"']").addClass("selected")

		$(".table .thead input[type=checkbox]").on('click', function()
		{
			$(this).closest(".table").find(".tbody .td input[type=checkbox]")
			.prop('checked', $(this).prop('checked') )
		})

		$(".table .ajax-action").on('click', function()
		{
			var $this = $(this),
				data = $(this).closest('.tr').data('ajax')
				data.action = $(this).data('action')

			if( !$(this).hasClass("confirm") || confirm($(this).data('confirm') || "Подтвердите действие!") )
